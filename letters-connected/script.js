...
function getNewLetter() {
  ...
  setCycleOrder();
  const currentLetter2 = cycleOrder[cycleIndex];
  setCycleOrder();
  const currentLetter3 = cycleOrder[cycleIndex];
  letterDisplay.textContent = currentLetter2.letter + currentLetter.letter + currentLetter3.letter;
...
function setOptions(options, currentLetter) {
  options.forEach((option, index) => {
    ...
    radio.addEventListener('change', function () {
      ...
      transcriptionDisplay.innerHTML = `<span class="letterTranscription"> - ${currentLetter.transcription} - ${currentLetter.letter} </span>`;
...
function setLogItems(isCorrect, currentLetter) {
  if (isCorrect) {
    loggedCorrectLetters.innerHTML += `<li>${letterDisplay.innerHTML}</li>`;
  } else {
    loggedWrongLetters.innerHTML += `<li>${letterDisplay.innerHTML}</li>`;
  }
}
